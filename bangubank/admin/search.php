...
$fileHandler = new FileHandler("../database/admins.json");

$email = $_SESSION['email'];
$admins = $fileHandler->loadData();
...
$transactions = $transitionFile->loadData();
$transactions = array_reverse($transactions);

$customersFile = new FileHandler("../database/users.json");
$customers = $customersFile->loadData();
...
if ($_SERVER['REQUEST_METHOD']== 'POST') {
  ...
  if (!$found) {
    $validation->addError('search_error', 'No customer found with the provided email.');
   }
...
<html
  ...
  <head>
    ...
    <title>Transactions of <?php echo $user_name; ?></title>
  </head>
  <body class="h-full">
      ...
      <div class="bg-sky-600 pb-32">
        ...
        <header class="py-10">
          <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <?php  
                if ($validation->hasErrors('search_error')){
                   echo $validation->getError('search_error');
                } else{
                    echo "Transactions of {$user_name}";
                }
            ?>
              ...
              <div class="sm:flex sm:items-center">
                <div class="sm:flex-auto">
                  <p class="mt-2 text-sm text-gray-700">
                    List of transactions made by <?= $user_name;?>.
                  </p>
                </div>
              </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                      ...
                      <tbody class="divide-y divide-gray-200 bg-white">
                        <?php
                            ...
                            foreach($transactions as $transaction) : 
                                if ($transaction["account"] == $user_email ):
                        ...
                        <tr>
                          ...
                          <td
                            class="whitespace-nowrap px-2 py-4 text-sm font-medium <?= $transaction['transitionType'] == "deposit"   ? "text-emerald-600" : "text-red-600"  ?> ">
                            <?= $transaction['transitionType'] == "deposit"  ? "+" : "-"  ?>$<?= $transaction['amount'] ?>
                          </td>
                          <td
                      ...
                      <tbody class="divide-y divide-gray-200 bg-white">
                        <?php foreach($transactions as $transaction) : 
                          if ( $transaction["toAccount"] == $user_email && $transaction["transitionType"] === "transfer"  ):
                        ?>
                        <tr>
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-800 sm:pl-0">
                          ...
                          <td
                            class="whitespace-nowrap px-2 py-4 text-sm font-medium <?= $transaction['transitionType'] == "transfer" ? "text-emerald-600" : "text-red-600"  ?> ">
                            <?= $transaction['transitionType'] == "transfer" ? "+" : "-"  ?>$<?= $transaction['amount'] ?>
                          </td>
                          <td
                            class="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
                            <?= $transaction['date'] ?>
