...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $amount = $_POST['amount'];
    $toAccountEmail = $_POST['email'];
    $found = false;
    foreach ($customers as $customer) {

      if ( $toAccountEmail ==$customer['email']) {
    ...
    if ($found) {
      $fileHandler = new FileHandler("../database/transactions.json");
      $transition = new Transition( $receiverName, $amount,'transfer',     $customer_email, $toAccountEmail);
      $transitionManager = new TransitionManager($fileHandler, $validation);
      $transitionManager->transferMoney($transition);
    }else{
